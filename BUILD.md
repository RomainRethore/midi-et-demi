...
1. Clique **« Mapping… »** : une fenêtre liste les actions (Lecture/Stop,
   Enregistrer, Effacer, Annuler, Métronome, Volume piste active, Piste 1-8).
2. Clique **« Apprendre »** sur une action, puis **bouge un pad / potard /
   bouton** de l'Oxygen → l'association est mémorisée (affichée « CC n » / « Note n »).
3. Ce contrôle déclenche désormais l'action (et ne joue plus de note).
   Le **X** efface une association.

> Mappe des **pads / potards / boutons de transport**, pas les touches du
> clavier (elles servent à jouer). Le mapping n'est pas encore sauvegardé entre
> deux sessions (ça viendra avec la sauvegarde de session).
